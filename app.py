...
if view_mode == "dangky":
    ...
    with col2:
        st.metric("Vận Chuyển", "Đang xử lý")
    with col3:
        st.metric("Lắp Đặt Xong", "Hoàn thành")
    with col4:
        st.metric("Tỷ Lệ Nghiệm Thu", "Thời gian thực")
    ...
    with st.form("form_hientruong"):
        ...
        if btn_gui:
            st.success(f"🎉 Đã gửi báo cáo thành công cho {doi_thuc_hien} tại {diem_lap_dat}! Trạng thái: {tinh_trang}.")
            st.balloons()
